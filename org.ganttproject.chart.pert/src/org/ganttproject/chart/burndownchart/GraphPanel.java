...
public class GraphPanel extends JPanel {
    ...
    private void drawIdealFlowLine(Graphics2D g2, double xScale, double yScale, int maxScore) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < numberYDivisions+1; i++) {
            ...
            int y1 = y0;
            if (tasksTotalDuration > 0) {
                g2.setColor(gridColor);
                g2.drawLine(padding + labelPadding + 1 + pointWidth, y0, getGraphWidth() - padding, y1);
                g2.setColor(Color.BLACK);
                String yLabel = (int) ((minScore + (maxScore - minScore) * (i * 1.0 / numberYDivisions)) ) + "";
        ...
        // and for x axis
        for (int i = 0; i < estimatedTime + 1; i++) {
            if (estimatedTime > 1) {
                int x0 = i * (getGraphWidth() - padding * 2 - labelPadding) / estimatedTime + padding + labelPadding;
                int x1 = x0;
                int y0 = 800 - padding - labelPadding;
                int y1 = y0 - pointWidth;
                if ((i % ((int) ((estimatedTime / 20.0)) + 1)) == 0) {
                    g2.setColor(gridColor);
